 ...
 * 
 * Copyright (c) 2024. 
 * Authors: Z. Leeper., K. Chandrasekhar
 * All rights reserved.
...
int main(int argc, char* argv[]) {
    ...
    if (!ec && (week < 1 || week > 3)) {
        fprintf(stderr, "Error: Week must be between 1 and 3 or left unspecified.\n");
        exit(EXIT_FAILURE);
    }

    if (ec && week == 1) {
        fprintf(stderr, "Error: Week must be at least 2 to test extra credit.\n");
        exit(EXIT_FAILURE);
    }
        ...
        for (size_t dirnum = 0; dirnum < num_testdirs; dirnum++) {
            ...
            for (int i = 0; i < num_cache_configs[dirnum]; i++,cachenum++) {
                for (size_t testnum = 0; testnum < test_sizes[dirnum]; testnum++){
                    ...
                    int pass = run_test(3, cache_configs[cachenum][0], cache_configs[cachenum][1], 
                                                cache_configs[cachenum][2], cache_configs[cachenum][3],
    ...
    if(ec) {
        ...
        char* testdir = week == 1 ? "testcases/charmv3/simple/" : "testcases/charmv3/hazard/";
